...
for i in range(0, N_UE):
    id = env.insert_ue(0, starting_position=(random.randint(0, env.x_limit - 1), random.randint(0, env.y_limit - 1), 1),
                       speed=10, direction=random.randint(0, 359))
    ue.append(id)
...
nr_bs1 = env.place_NR_base_station((3500, 1000, 40), 800, 2, 20, 16, 3, 100)
bs.append(nr_bs1)

nr_bs2 = env.place_NR_base_station((1000, 1000, 40), 800, 2, 20, 16, 3, 100)
bs.append(nr_bs2)

nr_bs3 = env.place_NR_base_station((2000, 3000, 40), 800, 2, 20, 16, 3, 100)
bs.append(nr_bs3)

drone_bs1 = env.place_DRONE_relay((3000, 1500, 150), nr_bs1, 800, 80, 16, 3)
bs.append(drone_bs1)

drone_bs2 = env.place_DRONE_relay((1000, 2000, 150), nr_bs2, 800, 80, 16, 3)
bs.append(drone_bs2)

drone_bs2 = env.place_DRONE_relay((2500, 3000, 150), nr_bs3, 800, 80, 16, 3)
bs.append(drone_bs2)
...
env.next_timestep()
'''
connected_num = np.zeros((len(bs), SIM_TIME))

# start_move = False
status = random.getstate()
# util.find_ue_by_id(0).disconnect_from_bs()
for cycle in range(0, SIM_TIME):
    print(
        "------------CYCLE %s------------" % cycle)
    print("\n")
    # random.shuffle(ue)
    ...
    for j in range(0, len(ue)):
        ...
        if ue_j.current_bs is not None:
    ...
    for i in range(0, len(bs)):
        connected_num[i, cycle] = len(util.find_bs_by_id(bs[i]).get_connected_users())
...
'''
axs[0].set_ylim(-10, max(max(load_sat), max(load_relay), max(load_bs)) + 50)
# plt.tight_layout()

# fig, axs = plt.subplots()
color_format = ['m', 'b', 'g', 'r', 'y', 'c', 'm']
line_format = ['-', '-.']

if len(bs) > len(color_format) * len(line_format):
    raise Exception("Need more formats to set the labels of Connected UEs figure")
for i in range(0, len(bs)):
    color = i % len(color_format)
    line = i // len(color_format)
    if util.find_bs_by_id(i).bs_type != "drone_relay":
        axs[1].plot(connected_num[i, :], color_format[color] + line_format[line], label="BS" + str(i))

# axs[1].plot(connected_sat, 'k', label="1")
# axs[1].plot(connected_relay, 'b-.', label="2")
# axs[1].plot(connected_bs, 'r--', label="3")
...
axs[1].set_ylim(-1, np.amax(np.amax(connected_num, axis=1)) + 1)
...
axs[2].set_ylim(min(res1) - 1, max(res1) + 1)
