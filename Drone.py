...
class DroneRelay:
    ...
    def compute_rsrp_drone(self, ue):
        ...
        return self.amplification + util.compute_rsrp(self, util.find_bs_by_id(self.linked_bs),
                                                      self.env) + self.antenna_gain - self.feeder_loss - util.compute_path_loss_cost_hata(
            ue, self, self.env)
...
class DroneBaseStation:
    ...
    def __init__(self, bs_id, total_prb, prb_bandwidth_size, number_subcarriers, numerology, antenna_power,
                 antenna_gain, feeder_loss, carrier_frequency, position, env):
        if position[2] > 200 or position[2] < 30:
            raise Exception("COST-HATA model requires BS height in [30, 200]m")
    ...
    def compute_nprb_NR(self, data_rate, rsrp):
        ...
        thermal_noise = constants.Boltzmann * 293.15 * list(NRbandwidth_prb_lookup[self.numerology][self.fr].keys())[
            list(NRbandwidth_prb_lookup[self.numerology][self.fr].values()).index(
                self.total_prb / (10 * 2 ** self.numerology))] * 1000000 * (self.compute_rbur() + 0.001)
    ...
    def request_connection(self, ue_id, data_rate, rsrp):
        ...
        if ue_id not in self.ue_pb_allocation:
            ...
            self.ue_pb_allocation[ue_id] = N_prb
            self.allocated_prb += N_prb
        print("BS %s 分配 %s PRB" % (self.bs_id, N_prb))
