...
class wireless_environment:
    ...
    def insert_ue(self, ue_class, starting_position=None, speed=0, direction=0):
        ...
        if starting_position is None:
            ...
            new_ue = ue.user_equipment(ue.ue_class[ue_class], ue_class, ue_id, starting_position, self, speed,
                                       direction)
    ...
    def place_NR_base_station(self, position, carrier_frequency, numerology, antenna_power, antenna_gain, feeder_loss,
                              available_bandwidth, drone=False):
        ...
        if available_bandwidth in NRbs.NRbandwidth_prb_lookup[numerology][fr]:
            ...
            if drone == False:
                new_bs = NRbs.NRBaseStation(len(self.bs_list),
                                            ...
                                            antenna_gain, feeder_loss, carrier_frequency, position, self)
            else:
        ...
        with ThreadPoolExecutor(max_workers=len(self.bs_list)) as executor:
            ...
            for i in range(0, len(self.bs_list)):
                ...
                if res > -1000000:  # if (res > util.MIN_RSRP):
                    rsrp[i] = res
        print(f"UE {ue_id} :")
        for index, value in rsrp.items():
            print(f"BS {index} 的 RSRP 值是 {value:.2f} dBm")
        return rsrp
