...
class user_equipment:
    ...
    def __init__(self, requested_bitrate, service_class, ue_id, starting_position, env, speed, direction):
        ...
        self.old_service_class = service_class
    ...
    def do_action(self, t):
        ...
        if t != 0 and self.old_bs != self.current_bs:
            self.handover_counter += 1
            print("发生切换： BS %s -> BS %s" % (self.old_bs, self.current_bs))
    ...
    def connect_to_bs_random(self):
        ...
        if len(available_bs) == 0:
            ...
            self.current_bs, self.actual_data_rate = self.env.request_connection(self.ue_id, self.requested_bitrate,
                                                                                 available_bs)
            ...
            if self.MATLAB == 1:
                ...
                self.actual_data_rate = util.find_bs_by_id(bs).request_connection(self.ue_id, self.requested_bitrate,
                                                                                  available_bs)
        ...
        print(
            "[CONNECTION_ESTABLISHED]: User ID %s is now connected to base_station %s with a data rate of %s/%s Mbps" % (
                self.ue_id, self.current_bs, self.actual_data_rate, self.requested_bitrate))
    ...
    def disconnect_from_bs(self):
        self.old_bs = self.current_bs
        if self.current_bs is not None:
            util.find_bs_by_id(self.current_bs).request_disconnection(self.ue_id)
            # print("[CONNECTION_TERMINATED]: User ID %s is now disconnected from base_station %s" % (
            # self.ue_id, self.current_bs))
            self.current_bs = None
            self.actual_data_rate = 0
        return
    ...
    def update_connection(self):
        ...
        # print(available_bs)
        if len(available_bs) == 0:
            print("[NO BASE STATION FOUND]: User ID %s has not found any base station during connection update" % (
                self.ue_id))
        elif self.current_bs in available_bs:
            self.actual_data_rate = util.find_bs_by_id(self.current_bs).update_connection(self.ue_id,
                                                                                          self.requested_bitrate,
                                                                                          available_bs)
            if self.actual_data_rate < self.requested_bitrate:
                print(
                    "[POOR BASE STATION]: User ID %s has a poor connection to its base station (actual data rate is %s/%s Mbps)" % (
                        self.ue_id, self.actual_data_rate, self.requested_bitrate))
                self.disconnect_from_bs()
                self.connect_to_bs()
            '''
            ...
            print("[BASE STATION LOST]: User ID %s has not found its base station during connection update" % (
                self.ue_id))
            self.disconnect_from_bs()
            self.connect_to_bs()

        print(
            "[CONNECTION_UPDATE]: User ID %s has updated its connection to base_station %s with a data rate of %s/%s Mbps" % (
                self.ue_id, self.current_bs, self.actual_data_rate, self.requested_bitrate))
    ...
    def reset(self, t):
        ...
        # self.service_class = self.old_service_class
        # self.lambda_exp = ue_class_lambda[self.service_class]
        # self.requested_bitrate = ue_class[self.service_class]
